                ...
                switch (aggFunc.toUpperCase()) {
                    case 'SUM':
                        groupResults[groupKey].sums[aggField] =
                            (groupResults[groupKey].sums[aggField] || 0) +
                            value;
                        break;
                    case 'MIN':
...
async function executeSELECTQuery(query) {
    try {
        ...
        if (hasAggregateWithoutGroupBy && fields.length == 1) {
            const selectedRow = {};
            selectedRow[fields[0]] = aggregatedOperations(
                fields[0],
                filteredData
            );
            return [selectedRow];
        ...
        return filteredData.map((row) => {
            ...
            fields.forEach((field) => {
                if (hasAggregateWithoutGroupBy) {
                    selectedRow[field] = aggregatedOperations(
                        field,
                        filteredData
                    );
                } else {
...
async function executeINSERTQuery(query) {
    const { table, columns, values, returningColumns } =
...
async function executeDELETEQuery(query) {
    ...
    if (whereClause.length > 0) {
        data = data.filter(
            (row) =>
                !whereClause.every((clause) => evaluateCondition(row, clause))
        );
    } else {
        data = [];
    }
